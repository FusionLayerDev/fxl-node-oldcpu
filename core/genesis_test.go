...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "custom block in DB, genesis == fusionlayer",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				customg.MustCommit(db)
				return SetupGenesisBlock(db, trie.NewDatabase(db), DefaultFusionLayerGenesisBlock())
			},
			wantErr:    &GenesisMismatchError{Stored: customghash, New: params.FusionLayerGenesisHash},
			wantHash:   params.FusionLayerGenesisHash,
			wantConfig: params.FusionLayerConfig,
		},
		{
...
func TestGenesisHashes(t *testing.T) {
	for i, c := range []struct {
		...
		{DefaultFusionLayerGenesisBlock(), params.FusionLayerGenesisHash},
	} {
		// Test via MustCommit
