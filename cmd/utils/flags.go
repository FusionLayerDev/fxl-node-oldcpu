...
var (
	...
	FusionLayerFlag = &cli.BoolFlag{
		Name:     "fusionlayer",
		Usage:    "FusionLayer mainnet",
		Value:    true,
		Category: flags.EthCategory,
	}
...
var (
	...
	NetworkFlags = append([]cli.Flag{MainnetFlag, FusionLayerFlag}, TestnetFlags...)
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		//if ctx.Bool(FusionLayerFlag.Name) {
		//	return filepath.Join(path, "fusionlayer")
		//}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.Bool(FusionLayerFlag.Name):
		urls = params.FusionLayerBootnodes
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(SepoliaFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		...
		// case ctx.Bool(FusionLayerFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		//	cfg.DataDir = filepath.Join(node.DefaultDataDir(), "fusionlayer")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, FusionLayerFlag, DeveloperFlag, RinkebyFlag, GoerliFlag, SepoliaFlag)
	...
	case ctx.Bool(FusionLayerFlag.Name):
		...
		cfg.Genesis = core.DefaultFusionLayerGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.FusionLayerGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(FusionLayerFlag.Name):
		genesis = core.DefaultFusionLayerGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
