...
func (ethash *Ethash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	if chain.Config().IsFusionLayer(header.Number) && header.MixDigest != (common.Hash{}) {
		return fmt.Errorf("mix digest should be zero value after fusionlayer fork, %v", header.MixDigest)
	}
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	switch {
	case config.IsFusionLayer(next):
		return calcDifficultyFusionLayer(time, parent)
...
// calcDifficultyFusionLayer is the difficulty adjustment algorithm. It returns
// the difficulty that a new block should have when created at time given the
// parent block's time and difficulty. The calculation uses the FusionLayer rules.
func calcDifficultyFusionLayer(time uint64, parent *types.Header) *big.Int {
	// https://github.com/ethereum/EIPs/issues/100.
	// algorithm:
	// diff = (parent_diff +
...
func (ethash *Ethash) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, fulldag bool) error {
	...
	if chain == nil || chain.Config().IsFusionLayer(header.Number) {
		...
		arr_result := fusionHash(hashSource)
		result = arr_result[:]
	} else {
		var digest []byte
...
// -----------------------------------------------------------------------------
// FusionLayer block reward logic (uncle rewards removed).

// Base miner reward
var baseReward = big.NewInt(9e18)

// Reduction logic constants
var reductionInterval = uint64(500000)
var reductionNumerator = big.NewInt(95)   // 95%
var reductionDenominator = big.NewInt(100)

// Development fund address
var DevelopmentFundAddress = common.HexToAddress("0x560eA05cc475396Cb3D957D55b5dA3E54d819a1e")

// Base development reward
var devReward = big.NewInt(2e18)

// accumulateRewards distributes block rewards according to the EtherPrime emission schedule
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {

	// Calculate number of reductions
	blockNumber := header.Number.Uint64()

	numReductions := uint64(0)
	if blockNumber > 0 {
		numReductions = (blockNumber - 1) / reductionInterval
	}

	// Calculate miner reward
	blockReward := new(big.Int).Set(baseReward)

	for i := uint64(0); i < numReductions; i++ {
		blockReward.Mul(blockReward, reductionNumerator)
		blockReward.Div(blockReward, reductionDenominator)
	}

	// Development reward: 2 coin per block, active until block 5,000,000
	...
	// Uncles intentionally not rewarded
}
