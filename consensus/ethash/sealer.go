...
func (ethash *Ethash) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	var (
		...
		isFusionLayer = chain == nil || chain.Config().IsFusionLayer(block.Header().Number)
	)
	for i := 0; i < threads; i++ {
		pend.Add(1)
		go func(id int, nonce uint64) {
			defer pend.Done()
			if isFusionLayer {
				ethash.mine_fusionhash(block, id, nonce, abort, locals)
			} else {
...
// fusionhash mine
func (ethash *Ethash) mine_fusionhash(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	...
	logger.Trace("Started ethash/fusionhash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
			// Mining terminated, update stats and abort
			logger.Trace("Ethash/fusionhash nonce search aborted", "attempts", nonce-seed)
		...
		default:
			...
			result := fusionHash(tryHashSource)
			...
			if powBuffer.SetBytes(result[:]).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("Ethash/fusionhash nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("Ethash/fusionhash nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
